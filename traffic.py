...
def get_route_info(r):
    ...
    for segment in results:
        ...
        this_time_hrs = sec_to_hrs(this_time)
    ...
    print '\nWAZE: Total Route Time (mins): ' + str(total_time / 60.00)
    print 'WAZE: Total Route Distance (miles)' + str(route_distance)
    print 'WAZE (PRIMARY NODE): Node Count: ' + str(node) + '\n'
...
def main_traffic():
    ...
    waze_resp = get_waze_resp(start_coords, end_coords) # obtain raw data from waze
    route_info = get_route_info(waze_resp)              # processed route data
    route = get_route(waze_resp)
    exportToJSON(route, 'raw_route_data.txt')
    plotOnMap(start_coords, route_info, 'main_traffic.html')                 # plot on google map webpage
...
def plotOnMap(start_coords, route_info, outputFile):
